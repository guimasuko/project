...
rj = rj %>%
  mutate(`Raz�o Florestal` = (n_forest/n_pixels))

texto_azul <- element_text(color = "#201c5c", family = "mono")

ggplot(rj) +
  geom_sf(aes(fill = `Raz�o Florestal`)) +
  ...
  labs(title = "Cobertura Vegetal",
         subtitle = "por munic�pio do estado do Rio de Janeiro",
         x = NULL, y = NULL)+
  scale_fill_viridis_c(option = 'D')
         
# Data frame final com os munic�pios e suas raz�es florestais
rj_ratio = rj %>%
  select(name_muni, name_state, `Raz�o Florestal`)
